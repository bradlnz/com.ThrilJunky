...
class MoreInfoController: UIViewController, UITableViewDataSource, UITableViewDelegate, ASVideoNodeDelegate, FusumaDelegate {
    ...
    func fusumaImageSelected(_ image: UIImage, source: FusumaMode) {
        DispatchQueue.main.async {
            let photoVC : PhotoViewController = PhotoViewController.init(image: image)
    ...
    func fusumaVideoCompleted(withFileURL fileURL: URL) {
        DispatchQueue.main.async {
            let videoVC : VideoViewController = VideoViewController.init(videoURL: fileURL)
                ...
                for item in snapshot.children {
                    ...
                    self.ref.child("businesses").queryOrdered(byChild: "uid").queryEqual(toValue: video.uid).observeSingleEvent(of: .value, with: { (snap) in
                        ...
                        for b in snap.children {
                            ...
                            self.latitude = Float(business.latitude)!
                            self.longitude = Float(business.longitude)!
    ...
    @IBAction func contribute(_ sender: Any) {
        DispatchQueue.main.async {
            ...
            fusuma.cropHeightRatio = 1.2// Height-to-width ratio. The default value is 1, which means a squared-size photo.
