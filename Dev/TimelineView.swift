...
open class TimelineView: UIView {
	...
	*/
	open var titleLabelColor: UIColor = UIColor(red: 255/255, green: 135/255, blue: 7/255, alpha: 1){
		didSet{
			setupContent()
		}
	}
	...
	*/
	...
	//fileprivate var imageViewer: JTSImageViewController?
	...
	*/
	...
	fileprivate func blockForTimeFrame(_ element: TimeFrame, imageTag: Int) -> UIView{
		...
		if let image = element.image{
			...
			button.tag = element.indexPath!
            button.addTarget(self, action: #selector(TimelineView.tapImage(_:)), for: UIControlEvents.touchUpInside)
    ...
    func tapImage(_ sender : UIButton){
        ...
        delegate?.tappedTimelineItem(indexPath: sender.tag)
...
protocol TimelineViewDelegate {
    func tappedTimelineItem(indexPath : Int)
}
